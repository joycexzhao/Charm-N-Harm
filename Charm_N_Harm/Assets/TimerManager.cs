...
public class TimeManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        int milliseconds = Mathf.FloorToInt((time % 1) * 10); // Get the first digit of milliseconds

        // Update the UI text to include one digit for milliseconds
        timerText.text = string.Format("Time: {0:00}:{1:00}.{2:0}", minutes, seconds, milliseconds);
    }
